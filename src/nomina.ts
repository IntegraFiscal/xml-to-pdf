...
function _i2w(n: number): string {
	...
	if (n < 100) {
		const u = n % 10;
		return u ? `${_T[Math.floor(n / 10)]} Y ${_U[u]}` : _T[Math.floor(n / 10)];
	}
	if (n === 100) return "CIEN";
	if (n < 1000) {
		const r = n % 100;
		return r
			? `${_H[Math.floor(n / 100)]} ${_i2w(r)}`
			: _H[Math.floor(n / 100)];
	}
	if (n < 1_000_000) {
...
function labelCell(text: string, primaryColor: string): TableCell {
	return {
		...
		bold: true,
		fontSize: 7,
		margin: [0, 1, 2, 1] as [number, number, number, number],
	};
}
...
function empleadoSection(
	...
	if (nombreEmpleado) {
		leftData.push([
			{ text: nombreEmpleado, style: ["subHeader"], color: primaryColor },
		]);
	}
	const addL = (label: string, value: string) => {
		if (value)
			leftData.push([
				{ text: [{ text: `${label} `, color: primaryColor }, { text: value }] },
			]);
	};
	...
	addR(
		"Tipo nómina",
		TIPOS_NOMINA[a(nomina, "TipoNomina")] ?? a(nomina, "TipoNomina"),
	);
	addR(
		"Período",
		`${a(nomina, "FechaInicialPago")} – ${a(nomina, "FechaFinalPago")}`,
	);
...
function buildPercepcionesTable(
	...
	const headers: TableCell[] = ["No.", "Concepto", "Gravado", "Exento"].map(
		(t) => headerCell(t, primaryColor),
	);

	const dataRows: TableCell[][] = list.map((p: XmlNodeInterface) => [
		valueCell(a(p, "Clave"), { alignment: "center", fillColor: bgGrayColor }),
		valueCell(TIPOS_PERCEPCION[a(p, "TipoPercepcion")] ?? a(p, "Concepto"), {
			fillColor: bgGrayColor,
		}),
		valueCell(formatCurrency(a(p, "ImporteGravado")), {
			alignment: "right",
			fillColor: bgGrayColor,
		}),
		valueCell(formatCurrency(a(p, "ImporteExento")), {
			alignment: "right",
			fillColor: bgGrayColor,
		}),
	]);
	...
	const totalRow: TableCell[] = [
		...
		{},
		{
			text: formatCurrency(a(percepciones, "TotalGravado")),
			alignment: "right",
			bold: true,
			fontSize: 7,
			border: [false, true, false, false],
		...
		{
			text: formatCurrency(a(percepciones, "TotalExento")),
			alignment: "right",
			bold: true,
			fontSize: 7,
			border: [false, true, false, false],
...
function buildDeduccionesTable(
	...
	const dataRows: TableCell[][] = list.map((d: XmlNodeInterface) => {
		...
		return [
			...
			valueCell(TIPOS_DEDUCCION[a(d, "TipoDeduccion")] ?? a(d, "Concepto"), {
				fillColor: bgGrayColor,
			}),
			valueCell(formatCurrency(a(d, "Importe")), {
				alignment: "right",
				fillColor: bgGrayColor,
			}),
		];
...
function percepcionesDeduccionesSection(
	...
	const leftContent = buildPercepcionesTable(
		percepciones,
		primaryColor,
		bgGrayColor,
	);
	...
	if (deducciones && deducciones.searchNodes("nomina12:Deduccion").length > 0) {
		const rightContent = buildDeduccionesTable(
			deducciones,
			primaryColor,
			bgGrayColor,
		);
...
function nominaInfoSection(
	...
	const mpText = metodoPago
		? `${metodoPago} – ${METODOS_PAGO[metodoPago] ?? metodoPago}`
		: "";
	const fpText = formaPago
		? `${formaPago} – ${FORMAS_PAGO[formaPago] ?? formaPago}`
		: "";
...
export class NominaCfdiTranslator {
	...
	translate(
		...
		const doc = this.inner.translate(
			data,
			documentOptions as any,
			catalogs as any,
			primaryColor,
			bgGrayColor,
		);
		...
		const nomina = comprobante.searchNode(
			"cfdi:Complemento",
			"nomina12:Nomina",
		);
		...
		content.splice(
			4,
			2,
			empleadoSection(nomina, cfdiReceptor, primaryColor, bgGrayColor),
			"\n",
		);

		// Percepciones | Deducciones in 2 columns
		const percDed = percepcionesDeduccionesSection(
			nomina,
			primaryColor,
			bgGrayColor,
		);
